...
export function RegisterModal({ open, onClose, onSwitchToLogin }: RegisterModalProps) {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    // Form verilerini sakla ve şartlar modalını aç
  ...
  const handleTermsAccept = async () => {
    ...
    // Email alanı boşsa Google sign-in, doluysa normal kayıt
    if (!pendingFormData.email) {
      // Google Sign-in
      await handleGoogleSignInAfterTerms();
    } else {
      // Normal Email/Password kayıt
      setLoading(true);
      try {
        await registerUser(pendingFormData);
        toast({
        ...
        // Modalları kapat ve formu temizle
  ...
  const handleGoogleSignIn = async () => {
    // Google sign-in için de şartları göster
    setPendingFormData({ 
      firstName: "", 
      lastName: "", 
      username: "", 
  ...
  const handleGoogleSignInAfterTerms = async () => {
        ...
        <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto" data-testid="modal-register">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              >
                {loading ? "Kayıt oluşturuluyor..." : "Şartları Oku & Kayıt Ol"}
              </Button>
            </form>
          </Form>
